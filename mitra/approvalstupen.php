...
$query = "SELECT * FROM stupen INNER JOIN mitra on stupen.id_mitra = mitra.id_mitra WHERE email = '$email'";
$hasil = mysqli_query($conn, $query);

echo '<div class="row row-cols-1 row-cols-md-2 g-4">';
while ($data = mysqli_fetch_array($hasil)) {
    $sql = "SELECT COUNT(program) AS jumlah FROM transaksi_mitra WHERE id_program='" . $data['id_stupen'] . "' AND program='stupen' AND status='pending'";
    ...
    <div class="col-xxl-4 col-md-5">
        <div class="card info-card sales-card">
            <div class="card-body">
                ...
                <div class="ps-3">
                    <a href="index.php?page=detailapproval<?php echo $data['program'];?>&id=<?php echo $data['id_stupen'];?>" style="font-size: 28px; display: flex; align-items: center;">
                        <h6 style="font-size: 22px; margin: 0;"><?php echo $data['judul_stupen']; ?></h6>
                    </a>
                    <p class="card-text mb-2 mt-4"><strong>Periode Kegiatan : </strong></p>
                    <p><?php echo "{$data['start_date']} - {$data['end_date']}"; ?></p>
                    <form class="d-flex justify-content-center" action="" method="post">
                        <input type="hidden" name="id_update" value='<?php echo $data['id_stupen'] ?>'>
